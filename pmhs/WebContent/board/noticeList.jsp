...
<body>
<form action="noticeBoardWriteForm.bo">	
	<section id = "listArea">
		...
		<h2> 공지 게시글 목록</h2>
			...
			<c:forEach var="article" items="${articleList }">
				<tr>
					...
					<td class = "td_subject">
						<a href = "noticeBoardContent.bo?num=${article.num }&pageNum=${pageInfo.currentPage}">${article.subject }</a>
					</td>
					<td class = "td_writer">
						${article.writer }
					</td>
	...
	<c:if test="${pageInfo.startPage > 20 }">
		<a href = "noticeBoardList.bo?pageNum=${pageInfo.startPage-20 }">[이전]</a>
	</c:if>
	...
	<c:if test="${pageInfoendPage < pageInfo.pageCount }">
		<a href = "noticeBoardList.bo?pageNum=${pageInfo.startPage+20 }">[다음]</a>
	</c:if>
	</section>
	</c:if>
	</form>
