package pmhs.web.pcInfo.controller;
...
import pmhs.web.pcInfo.action.PCDeclareProAction;
import pmhs.web.pcInfo.action.PCSelectFormAction;

/**
 * Servlet implementation class PCInfoFrontController
 */
@WebServlet("*.pc")
public class PCInfoFrontController extends HttpServlet {
    ...
    protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("/pcDeclarePro.pc")) { // 고장 PC 리스트
			action = new PCDeclareProAction();
			try {
				forward = action.execute(request, response);
			} catch (Exception e) {
				// TODO: handle exception
